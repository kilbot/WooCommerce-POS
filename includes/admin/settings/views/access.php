...
<p class="update-nag" style=font-size:13px;margin:0;">
	...
	<?php printf( __( 'For more information please visit <a href="%1$s" target="_blank">%1$s</a>', 'woocommerce-pos' ), wcpos_doc_url( 'settings/pos-access.html' ) ); ?>
</p>

<div class="wc_pos-access">
	<ul class="wc_pos-access-tabs">
		<?php $data = $this->get();
		if ( $data ): foreach ( $data['roles'] as $slug => $role ): ?>
				...
				<ul>
					<?php if ( isset( $this->caps ) ): foreach ( $this->caps as $key => $caps ): ?>
						<?php if ( $caps ): foreach ( $caps as $cap ): ?>
							<li>
								<input type="checkbox"
								       name="roles.<?php echo esc_attr( $slug ); ?>.capabilities.<?php echo esc_attr( $key ); ?>.<?php echo esc_attr( $cap ); ?>"/>
								<?php echo $cap; ?>
							</li>
						<?php endforeach; endif; ?>
					<?php endforeach; endif; ?>
