...
<h3><?php /* translators: woocommerce */
	_e( 'Checkout Process', 'woocommerce' ); ?></h3>
...
<table class="wc_pos-form-table">
	...
	<tr class="order_status">
		<th scope="row">
			...
			<img title="<?php esc_attr_e( 'Change the default order status for POS sales', 'woocommerce-pos' ) ?>"
		...
		<td>
			<select name="order_status" id="order_status">
				...
				if ( $statuses ): foreach ( $statuses as $status => $status_name ): ?>
	...
	<tr class="order_emails">
		<th scope="row">
			...
			<img title="<?php esc_attr_e( 'Send WooCommerce notification emails for POS orders', 'woocommerce-pos' ) ?>"
			     src="<?php echo WC()->plugin_url(); ?>/assets/images/help.png" height="16" width="16"
			     data-toggle="wc_pos-tooltip">
		</th>
		<td>
	...
	<tr class="receipt_printing">
		...
		<td>
			...
			<label
				for="auto_print_receipt"><?php _e( 'Automatically print receipt after checkout', 'woocommerce-pos' ); ?></label>
		</td>
	</tr>
...
<h3><?php /* translators: woocommerce */
	_e( 'Payment Gateways', 'woocommerce' ); ?></h3>

<p>
	<?php _e( 'Installed gateways are listed below. Drag and drop gateways to control their display order at the Point of Sale. ', 'woocommerce-pos' ); ?>
	<br>
...
<p class="update-nag" style="font-size:13px;margin:0;">
	...
	<?php printf( __( 'For more information please visit <a href="%1$s" target="_blank">%1$s</a>', 'woocommerce-pos' ), wcpos_doc_url( 'support/security.html' ) ); ?>
</p>
	...
	<tr>
		...
		<td>
			<table class="wc-gateways widefat sortable" cellspacing="0">
				<thead>
				<tr>
					<th scope="col"><?php /* translators: woocommerce */
						_e( 'Default', 'woocommerce' ) ?></th>
					<th class="gateway-name" scope="col"><?php /* translators: woocommerce */
						_e( 'Gateway', 'woocommerce' ) ?></th>
					<th class="gateway-id" scope="col"><?php /* translators: woocommerce */
						_e( 'Gateway ID', 'woocommerce' ) ?></th>
				...
				<?php foreach ( $this->load_gateways() as $gateway ) : ?>
					<tr id="gateway_<?php esc_attr_e( $gateway->id ) ?>">
						...
						<td style="display:none"
						    class="gateway-description"><?php echo esc_html( $gateway->get_description() ); ?></td>
						<td style="display:none" class="gateway-icon" data-icon="<?php echo $gateway->has_icon; ?>"
						    data-show="<?php echo $gateway->show_icon; ?>">
							<?php echo esc_html( $gateway->get_icon() ); ?>
						</td>
						<td class="gateway-id"><?php echo esc_html( $gateway->id ) ?></td>
						<td>
							<?php if ( $gateway->enabled == 'yes' ): ?>
								<span class="status-enabled" data-toggle="wc_pos-tooltip"
								      title="<?php /* translators: woocommerce */
								      esc_attr_e( 'Enabled', 'woocommerce' ); ?>"></span>
							<?php else: echo '-'; endif; ?>
						</td>
						<td>
							<?php if ( $gateway->pos ): ?>
							...
							<?php else: ?>
								<span class="status-disabled" data-toggle="wc_pos-tooltip"
								      title="<?php esc_attr_e( 'Upgrade to Pro', 'woocommerce-pos' ); ?>"></span>
							<?php endif; ?>
						</td>
						<td>
							<?php if ( $gateway->pos ): ?>
								<a class="button gateway-settings" data-gateway="<?php echo $gateway->id ?>"
								   href="#"><?php /* translators: woocommerce */
									_e( 'Settings', 'woocommerce' ) ?></a>
							<?php endif; ?>
						</td>
						<td>
							<?php if ( ! in_array( $gateway->id, array( 'pos_cash', 'pos_card' ) ) ): ?>
								<a class="button"
								   href="<?php echo admin_url( 'admin.php?page=wc-settings&tab=checkout&section=' . strtolower( get_class( $gateway ) ) ) ?>"><?php /* translators: woocommerce */
									_e( 'Settings', 'woocommerce' ) ?></a>
							<?php endif; ?>
						</td>
					</tr>
...
<table id="tmpl-gateway-settings-modal" style="display:none">
	<tr>
		<th scope="row">
			<label for="title"><?php /* translators: wordpress */
				_e( 'Title' ); ?></label>
			<img data-toggle="wc_pos-tooltip" title="<?php _e( 'Payment method title.', 'woocommerce-pos' ); ?>"
	...
	<tr>
		<th scope="row">
			<label for="description"><?php /* translators: wordpress */
				_e( 'Description' ); ?></label>
			<img data-toggle="wc_pos-tooltip"
			     title="<?php _e( 'Payment method description that will be shown in the POS.', 'woocommerce-pos' ); ?>"
			     src="<?php echo esc_url( WC()->plugin_url() ); ?>/assets/images/help.png" height="16" width="16"/>
		</th>
