...
class Menu {
	...
	public function admin_menu() {
		...
		// adjust submenu
		global $submenu;
		$pos_submenu       = &$submenu[ \WCPOS\PLUGIN_NAME ];
		$pos_submenu[0][0] = __( 'Upgrade to Pro', 'woocommerce-pos' );
		$pos_submenu[1][2] = wcpos_url();
	}
	...
	public function menu_order( $menu_order ) {
		...
		$pos = array_search( \WCPOS\PLUGIN_NAME, $menu_order );

		if ( $woo !== false && $pos !== false ) {
			// rearrange menu
			unset( $menu_order[ $pos ] );
			array_splice( $menu_order, ++$woo, 0, \WCPOS\PLUGIN_NAME );
