...
<?php foreach( $this->settings as $setting ): if($setting->current_user_authorized): ?>
	...
	<?php $data = $setting->get_data(); if( $data ): ?>
		<script type="text/javascript">
			var wc_pos_settings_<?php echo $setting->id ?> = <?php echo json_encode($data); ?>
		</script>
	<?php endif; ?>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h1><?php /* translators: wordpress */ _e( 'Title' ); ?></h1>
				<i class="icon icon-times" data-action="close" title="<?php /* translators: wordpress */ _e( 'Close' ); ?>"></i>
			</div>
			<div class="modal-body"></div>
			<div class="modal-footer">
				<p class="response"
					 data-success="<?php /* translators: woocommerce */ _e( 'Your changes have been saved.', 'woocommerce' ); ?>"
					 data-error="<?php /* translators: woocommerce */ _e( 'Sorry, there has been an error.', 'woocommerce' ); ?>">
				</p>
