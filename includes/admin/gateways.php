...
class Gateways {
	...
	public function __construct() {
		add_filter( 'woocommerce_payment_gateways_setting_columns', array(
			$this,
	...
	public function pos_status( $gateway ) {
		$settings = wcpos_get_option( 'checkout', 'enabled' );
		$enabled  = is_array( $settings ) ? array_keys( $settings, true ) : array();

		echo '<td class="pos_status">';
		if ( in_array( $gateway->id, $enabled ) ) {
			echo '<span class="status-enabled tips" data-tip="' . /* translators: woocommerce */
			     __( 'Enabled', 'woocommerce' ) . '">' . /* translators: woocommerce */
			     __( 'Enabled', 'woocommerce' ) . '</span>';
		} else {
			echo '-';
		}
		echo '</td>';
