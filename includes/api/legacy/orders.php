...
class Orders {
	...
	public function create_order_data( array $data ) {
		...
		if ( version_compare( WC()->version, '2.4', '<' ) && isset( $data['fee_lines'] ) ) {
		...
		if ( $customer_id ) {
			$data['billing_address']  = $this->get_customer_details( $customer_id, 'billing' );
...
//    $this->delete_order_items($order_id);
	...
	public function process_payment( $order_id, $data ) {
		...
		if ( isset( $gateways[ $gateway_id ] ) ) {
			...
			$this->payment_failure( $gateway_id, $order_id, '' );
		}
	...
	public function payment_complete( $order_id ) {
		...
		if ( $order->status == 'processing' ) {
			...
			$order->update_status( wcpos_get_option( 'checkout', 'order_status' ), $message );
		}
...
//    $order_data['shipping_address'] = $this->filter_address($order_data['shipping_address'], $order, 'shipping');
		...
		// fixed in WC 2.4+
		if ( version_compare( WC()->version, '2.4', '<' ) && wcpos_get_option( 'products', 'decimal_qty' ) ) {
	...
	private function add_payment_details( $order, array $payment_details ) {
		...
		if ( isset( $payment_details['method_id'] ) && $payment_details['method_id'] == 'pos_cash' ) {
			$payment_details['method_pos_cash'] = Cash::payment_details( $order );
		}
	...
	public function woocommerce_email( WC_Emails $wc_emails ) {
		...
		if ( ! wcpos_get_option( 'checkout', 'customer_emails' ) ) {
			$this->remove_customer_emails( $wc_emails );
		}

		if ( ! wcpos_get_option( 'checkout', 'admin_emails' ) ) {
	...
	public function serve_request( $served, $result, $server ) {
		...
		if ( $dummy ) {
			...
			// dummy data
			$result = array(
				'orders' => json_decode( file_get_contents( \WCPOS\PLUGIN_PATH . 'includes/dummy-data/legacy/v3/order.json' ) )
			);
