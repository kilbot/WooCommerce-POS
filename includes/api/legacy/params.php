...
class Params extends WC_API_Resource {
	 ...
	 * @return array
	 */
	public function get_params( $wcpos_admin = null ) {
		if ( $wcpos_admin ) {
			$params = apply_filters( 'woocommerce_pos_{$wcpos_admin}_params', $this->common_params() + $this->admin_params(), $this );
		} else {
	...
	private function frontend_params() {
		return array(
			'auto_print'    => wcpos_get_option( 'receipts', array(
				'section' => 'receipt_options',
				'key'     => 'auto_print_receipt'
			) ),
			'denominations' => i18n::currency_denominations(),
			'discount_keys' => wcpos_get_option( 'cart', 'discount_quick_keys' ),
			'fee'           => wcpos_get_option( 'cart', 'fee' ),
			'hotkeys'       => wcpos_get_option( 'hotkeys', 'hotkeys' ),
			'menu'          => $this->menu(),
			'shipping'      => $this->shipping(),
			'tabs'          => wcpos_get_option( 'products', 'tabs' ),
			'tax'           => $this->tax(),
	...
	private function admin_params() {
		return array(
			...
			'nonce'                  => wp_create_nonce( \WCPOS\PLUGIN_NAME ),
		);
	}
	...
	private function customers() {
		...
		$user_id = wcpos_get_option( 'customers', 'logged_in_user' ) ?
			get_current_user_id() :
			wcpos_get_option( 'customers', 'default_customer' );
	...
	private function shipping() {
		$shipping           = wcpos_get_option( 'cart', 'shipping' );
