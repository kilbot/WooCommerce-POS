...
class Ajax {
	...
	public function __construct() {
		...
		$ajax_events = array(
			'toggle_legacy_server' => '\WCPOS\Status'
		);

		foreach ( $ajax_events as $ajax_event => $class ) {
			// check_ajax_referer
			add_action( 'wp_ajax_wcpos_' . $ajax_event, array( $this, 'check_ajax_referer' ), 1 );
			// trigger method
			add_action( 'wp_ajax_wcpos_' . $ajax_event, array( $class, $ajax_event ) );
		}
	}
